...
Memory is a core Junior feature. It stores durable facts, recalls relevant facts
...
- `createMemory`, `archiveMemory`, `listMemories`, and `searchMemories` are
  model-visible tools registered by `registration.ts`. `archiveMemory` can forget
...
- The Drizzle schema in `../../db/schema/memory.ts` and packaged migrations define the
  database contract.
...
- `AI_MEMORY_MODEL` or `createApp({ memory: { modelId } })` selects the structured
  review model.
- `createApp({ memory: { disableRecall: true } })` disables automatic prompt recall.
- `createApp({ memory: { disableExtraction: true } })` disables passive session
...
- Generate schema changes with `pnpm --filter @sentry/junior db:generate`.
